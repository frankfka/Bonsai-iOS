...
// TODO: make a helper for publisher -> sink -> store
func homeScreenDidShowMiddleware() -> Middleware<AppState> {
...
func homeScreenInitMiddleware(logService: LogService) -> Middleware<AppState> {
    return { state, action, cancellables, send in
        switch action {
...
private func initHomeScreen(logService: LogService, for user: User, in category: LogCategory?, since date: Date?) -> AnyPublisher<AppAction, Never> {
