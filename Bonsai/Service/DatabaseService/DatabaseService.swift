...
protocol DatabaseService {
    ...
    func getLogs(for user: User, in category: LogCategory?, since beginDate: Date?, toAndIncluding endDate: Date?,
                 limit: Int?, startingAfterLog: Loggable?, offline: Bool) -> ServicePublisher<[Loggable]>
        ...
        */
        let future = ServiceFuture<[Loggable]> { promise in
            let logsFromRealm = self.realmService.getLogs(
                    ...
                    limit: limit,
                    startingAfterLog: startingAfterLog
            )
            if offline {
            ...
            self.firestoreService.getLogs(for: user, in: category, since: beginDate, toAndIncluding: endDate,
                    limitedTo: limit, startingAfterLog: startingAfterLog) { result in
